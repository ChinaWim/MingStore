...
<div class="container">
	<div class="row">
		<div
			...
			<a href="${pageContext.request.contextPath}/product?action=productList&cid=${product.cid}currentPage=1">首页&nbsp;&nbsp;&gt;</a>
			<a href="./蔬菜分类.htm">${product.cid}&nbsp;&nbsp;&gt;</a>
			<a>${product.pname}</a>
		</div>
				...
				<div
					...
					<div style="margin: 20px 0 10px 0;; text-align: center;">
						<input style="background: url('./images/product.gif') no-repeat scroll 0 -600px rgba(0, 0, 0, 0); height: 36px; width: 127px;"
							   value="加入购物车" type="button" onclick=" inCart() ">
						&nbsp;收藏商品
					</div>
				</div>
				<div>
					<a href="${pageContext.request.contextPath}/product?action=productList&cid=${product.cid}&currentPage=${currentPage}">返回列表</a>
				</div>
			</div>
		</div>
		<div class="clear"></div>
...
<script>
    function inCart() {
			$.post(
				"${pageContext.request.contextPath}/product?action=productCart",
				{"quantity":$("#quantity").val(),"pid":"${product.pid}"}
			);
			alert("成功添加!");
    }
</script>
