...
public class OrdersServiceImpl implements OrdersService {
    ...
    public boolean deleteOrders(String oid){
        ...
        try {
            ...
            JDBCUtils.startTransaction();
            //先删除 订单项
            boolean successDelete = orderitemService.deleteOrderitem(oid);
            if(successDelete){
                //再删除订单
                queryRunner.update(JDBCUtils.getConnection(),sql,oid);
            }
        } catch (Exception e) {
            e.printStackTrace();
            try {
                JDBCUtils.rollback();//事务回滚
            } catch (SQLException e1) {
                e1.printStackTrace();
            }
        }finally {
            try {
                JDBCUtils.commitAndRelease(); //事务提交
            } catch (SQLException e) {
                e.printStackTrace();
            }
        }
